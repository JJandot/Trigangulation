...
class ZoneSaisirPointsAfficherSegments extends JPanel  {

	/** Creation de la zone d'affichage. */
	ZoneSaisirPointsAfficherSegments()
	{
...
class CanvasSaisirPointsAfficherSegments extends JPanel implements MouseListener, MouseMotionListener {
	...
	private void drawPoints(Graphics g) {
		for (int n = 0; n < points.size(); n++) {
			...
			g.drawString(p.right ? "r" : "l", (int)p.x + 20, (int)p.y);
			g.drawString(Integer.toString(p.flag), (int)p.x + 30, (int)p.y);
		}
	}
